...
class TestCollaborativeTool:
    ...
    @pytest.mark.asyncio
    async def test_user_with_notebook_file_sets_awareness(
        self, mock_user_dict, mock_global_awareness, mock_ydoc
    ):
        ...
        with patch(
            ...
            mock_ydoc.awareness.set_local_state_field.assert_called_once_with(
                "user", mock_user_dict
            )

    @pytest.mark.asyncio
    async def test_user_with_non_notebook_file_only_sets_global_awareness(
        ...
        with patch(
            ...
            # Verify global awareness was set
            mock_global_awareness.set_local_state.assert_called_once_with(
                {"user": mock_user_dict, "current": "test_file.py", "documents": ["test_file.py"]}
            )
    ...
    async def test_file_path_detection_from_kwargs(self, mock_user_dict, mock_global_awareness):
        ...
        with patch(
            ...
            mock_global_awareness.set_local_state.assert_called_once_with(
                {"user": mock_user_dict, "current": "test.py", "documents": ["test.py"]}
            )

    @pytest.mark.asyncio
    async def test_file_path_detection_from_positional_args(
        self, mock_user_dict, mock_global_awareness
    ):
    ...
    @pytest.mark.asyncio
    async def test_function_without_file_path_parameter(
        self, mock_user_dict, mock_global_awareness
    ):
        ...
        with patch(
            ...
            mock_global_awareness.set_local_state.assert_called_once_with(
                {"user": mock_user_dict, "current": "", "documents": []}
            )
    ...
    async def test_notebook_awareness_error_handling(self, mock_user_dict, mock_global_awareness):
        ...
        with patch(
            ...
            mock_global_awareness.set_local_state.assert_called_once_with(
                {"user": mock_user_dict, "current": "test.ipynb", "documents": ["test.ipynb"]}
            )
    ...
    async def test_global_awareness_error_handling(self, mock_user_dict):
        ...
        with patch(
            "jupyter_ai_tools.utils.get_global_awareness",
            side_effect=Exception("Global awareness error"),
        ):
    ...
    async def test_complex_function_signature(self, mock_user_dict, mock_global_awareness):
        ...
        with patch(
            ...
            result = await complex_func(
                "first", "test.py", "extra1", "extra2", key1="value1", key2="value2"
            )
